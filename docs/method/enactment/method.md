...
> `../library/dependency-contract.md`, `../library/adr-mechanism.md`, `../library/standards-charter.md`, `../library/seed-manifest.md`, `../library/resources/`) **composed** into our way of working and shown
...
| Method Isolation (isolate an entangled endeavour's method from its product — the export-side cut) | Way of Working | ritual (mechanizable core: zero-product-literal grep + classification gate) | ✓ (when extracting a method, or before a bidirectional transfer channel) | ✅ `../library/practices/method-isolation.md` (L3 only — generic; klartext's F0→F3 run is the worked example in `method-seed-export-plan.md`) |
