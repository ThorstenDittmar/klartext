...
| **Memory pinning is machine-wide — migrate to project-specific scoping later** — `autoMemoryDirectory` in the user-global `~/.claude/settings.json` redirects ALL sessions on the machine to the klartext team memory | user decision (2026-06-10, Stage 1): accepted deliberately because klartext is currently the only project on this machine — "das kann sich aber ändern" | DevOps (settings mechanics) + OE (rollout) | **Closed (2026-06-15)** | The Stage-1 precondition ("revisit once project settings actually load") was met by ADR-0010/0011 — project `.claude/settings.json` now loads in the app. **Resolved 2026-06-13 (PR #99):** a committed project-scoped `autoMemoryDirectory` pins the team memory; empirically verified it beats user-global after trust (refutes the schemastore "ignored in checked-in settings" claim); `setup.sh` now *cleans* the user-global keys instead of writing them. **Cutover still pending** — physically removing the user-global keys is gated on all 9 other worktrees + the `~/klartext` main checkout picking up the committed pin via rebase-on-main (an OE operating-model rollout, like #100); until then user-global stays as the fallback (same path) so no worktree drops to a lonely per-cwd store. The leak (foreign projects see team memory) persists only until the cutover — pre-existing, low priority. OE closes this row when DevOps reports cutover-complete. **CLOSED 2026-06-15:** DevOps pulled the cutover — user-global `autoMemoryEnabled` + `autoMemoryDirectory` removed from `~/.claude/settings.json` (backup `settings.json.bak-cutover-20260615T075516Z`), the committed project pin now carries alone (empirically beats user-global after trust). Gate (11/11 worktrees pinned) reached 2026-06-15; the last three converged ux → narrative → qa. Foreign-project leak gone. #4 (autoMemory strand) complete. |
...
| **`pre-compact` skill renamed to `anchor` + successor-seed folded in** — the session-safeguard ritual's name was bound to `/compact`, which is no longer the loss-free path (we use `/clear`); the name was a stale-assumption artifact (the very RC4 class the Environment Work Product guards) | User noticed the name no longer fits ("wir machen es nicht vor compact") while asking for the loss-free restart sequence (2026-06-13); the seed step lived only as playbook prose, not in the skill — which is why the sequence kept being forgotten | OE (method, skill content, reference sweep) + DevOps (machine-local install + versioning) | **In Use — content landed (PR #104, merged 2026-06-14); `anchor` installed at `~/.claude/skills/` + stale `pre-compact` removed (DevOps); skill-sync governance pending (DevOps PR)** | `anchor` names the *essence* (anchor volatile chat knowledge to durable ground; on a restart drop an anchor the successor re-attaches to) — anlass-neutral, so it can't go stale the way `pre-compact` did. Change set: (1) new **versioned** skill `docs/superpowers/skills/anchor.md` (the old one lived only machine-local at `~/.claude/skills/`, outside git — the anti-loss skill was itself loss-prone); (2) **conditional** successor-seed step (restart mode only — skipped on a mid-session checkpoint to avoid stale handoff notes); (3) closing points at `/clear` not `/compact`, with the discard-vs-summarize rationale inline; (4) forward-looking reference sweep (`method.md` element register with retirement note "formerly Pre-Compact Capture", `knowledge-routing.md`, `agent-onboarding.md`, `practices/retrospective.md`, `agents/oe/*`). Historical records (ADR-0010, external-review docs, dated retro learning) left untouched — they record the old name correctly for their time. **DevOps briefing:** install `anchor` at `~/.claude/skills/`, remove the stale `pre-compact`, and bring cross-agent ritual skills under version control + distribution (governance gap). **Briefing to Hannibal:** his `claude.md` still references `pre-compact` (his Hoheitswissen — his edit). Two latent gaps surfaced and named: the restart ritual was not codified in one place, and a shared ritual skill lived outside git. |

| **App has no auto-rebase at session start — worktrees drift from `main`** — the terminal launcher (`start-agent.sh`) rebased the worktree on `origin/main` at every session start; the app does not. Worktrees therefore drift from `main` and only converge via a manual rebase or a PR merge — which is exactly what made the autoMemory cutover a manual, slow rollout, and recurs on every committed `settings.json` / hook / allowlist change | Surfaced during the autoMemory cutover rollout (2026-06-13): only 1/11 worktrees carried the committed pin; the rest lagged `main` 8–27 commits with no auto-sync. OE + DevOps | **Identified** | **Leitfrage: warnen statt automatisch rebasen.** Direction (OE + DevOps, 2026-06-13): **drift WARNING, not auto-rebase** — per our enforcement hierarchy (mechanical *detection* + human *action*). Auto-rebase at start is rejected as too destructive: worktrees sit on feature branches with WIP / open PRs, an automatic `rebase origin/main` can trigger conflicts or reorder work, and worktrees share one object store (`index.lock` contention — the same footgun `morning.sh` had). Design (DevOps): a SessionStart hook does a light `git fetch` + compares HEAD to `origin/main`; if the worktree is N commits behind **and** the delta touches committed `settings.json`/hooks/allowlist, it prints a hint ("worktree N behind main; committed settings may be stale — rebase when safe") — never touching the working tree. **Synergy: bundle with G2** (the Local-mode / memory-pin health check OE already approved) into ONE SessionStart/health check — *one hook, three invariants: Local? pin resolved? worktree current?* DevOps brings the hook-design side; OE owns the operating-model framing. **Practice cut (OE, method authority, 2026-06-14):** registered as **ONE** Practice **„Drift Awareness"** (advances the Way-of-Working Alpha), NOT two near-identical cards — same Alpha, same Activity Space, same mechanism (SessionStart fetch+compare hook; action half = `klartext converge`, formerly `klartext worktree-sync`). The detection sharpness is a **Port (drift-detector) with two Adapters**: **L1** (commit-count, coarse) and **L2** (shared-layer-weighted: warns harder when the lag touches committed `settings.json`/hooks/pins). Start at **L1**. The L1→L2 move is **not** a "state transition of the Practice" (a Practice has no Alpha-style lifecycle) — it is a **new Improvement Step instance** (own register row) that swaps the adapter, justified by Monitor evidence. **Monitor-Owner = OE** (falsifiable contract): *Signal* = latency between a shared-layer change landing on `main` and the worktrees carrying it; *"L1 doesn't carry"* = coarse warnings demonstrably fail to produce timely convergence of shared-layer changes (warning fatigue / signal lost in noise) → promote to L2 with rationale. DevOps builds L1 + the port seam, citing ADR-0012 (guarded voluntary convergence). Next: DevOps PR. |

| **Channel consolidation — inbox is the standing channel, even though the app channel is back** — with the return to the desktop app (2026-06-14) the app messaging fabric (`ccd_session_mgmt`) is available again, so cross-agent messaging now has *two* possible channels (file inbox `scripts/inbox.sh` + app messages). Risk: drifting into two channels by accident | Surfaced during the app-return work (2026-06-14): OE flagged twice that both channels are now live; the user kept using the inbox, but the choice was never recorded as a deliberate decision | OE | **Decided (2026-06-14)** | **Decision (user, by practice): „Inbox is the floor, app is the doorbell."** The file inbox is the single **channel of record** — anything actionable or persistent (briefings, approval-requests, handoffs, decisions) **must** land in the recipient's inbox. The app direct-message (`ccd_session_mgmt`) is allowed only as a live *nudge* on top of an inbox entry, or for purely ephemeral clarification with no action/persistence — **never** the sole carrier of an actionable item. Reconciliation runs over the inbox, so reading the inbox can never miss actionable work. This answers all three open questions: (1) *one channel of record, not two by accident* — the app is a courtesy bell, not a second mailbox; (2) *when is app preferable* — immediacy, when the item is also in the inbox, or pure ephemeral clarification; (3) *anti-drift* — the inbox is the mandatory floor. **Why (evidence, 2026-06-14, precise after fact-check):** the actionable item „#111 (ADR-0012) needs OE gate + merge" **never landed in OE's inbox** — SA's inbox notice about ADR-0012 went to *DevOps* (correctly, for the build), and DevOps's „ball at OE" reached OE only through the user's verbal relay; OE learned of the merge-blocking work solely via the user, not via the floor. (#110's gate request, by contrast, **did** arrive via OE's inbox and was handled — so the failure was a **missing / mis-addressed inbox deposit**, not "the wrong channel" as such and not a delivery bug. The first wording of this evidence overstated it as "reached only via app channel"; corrected here.) „Inbox by choice" as a mere aspiration did **not** hold; hence the inbox is made the binding floor, not just the default. Corollary lesson: verify the recipient slug when sending. Recorded home of the operational rule: `knowledge-routing.md` (loaded by every agent) + `agents/oe/claude.md`; memory [[oe-direct-message-autonomy]] updated. Routed to all agents. Principle *„the user is the channel"* (governs reading) is unchanged. |
| **Two-mode consistency for rolling out way-of-working changes** — pure ADR-0012 *eventual* convergence is unsafe for **breaking** method changes: a drifted agent runs a now-broken ritual before it ever converges | The 2026-06-14 anchor breakage: a way-of-working change landed on `main`, worktrees drifted, the anchored commands/paths broke; the team ran a manual stop-the-world (secure → update → resync) to recover | OE (method) + DevOps (mechanism) | **Decided (2026-06-14)** | **Decision (user): two-mode consistency — *rolling default, stop-the-world barrier for breaking changes.*** A condition on ADR-0012's Consistency axis: *eventual* by default; *strong/barrier* when the change is breaking for a **drifted** agent. **Trigger:** every way-of-working PR classifies "breaking for a drifted agent?" explicitly (no silent default; uncertainty → breaking, fail-safe). **Lever:** expand–contract first (alias old→new in a transition window) shrinks the barrier set. **Verortung (both, split):** the *decision* → **ADR-0012 addendum** (SA-authored, OE gates); the *procedure* → new Practice **Controlled Method Rollout** (`practices/controlled-method-rollout.md`: Stabilize&Halt=anchor → Update+bootstrap → Resync&Verify=converge+drift-check). **Mechanization (DevOps):** classification gate at PR + the SessionStart drift/G2 check as "verify current before resume". Meta: these additive method-doc changes are themselves backward-compatible → rolling, no barrier. Next: OE briefs SA (addendum); DevOps builds the gate + drift check (PR2). |
| **Memory-Substrate dependency modeled as a contract** — the storage mechanics (team memory + inbox + pin) is a central IT dependency the whole way of working sits on, yet was unmodeled (unlike the Claude app / GitHub); every 2026-06-14 friction (cutover, #108, drift, concurrent-consolidation race) was an ad-hoc substrate issue | User raised the seam: "not really external, but still a dependency on an IT component — our storage mechanics" | OE (method) + DevOps (mechanism/empirical) | **Decided (2026-06-14)** | **Decision (user): model it lean — a Dependency Contract + health-check, not a full standalone model.** The substrate is a **seam** (three faces): uncontrolled (app autoMemory resolution, `~/.claude`, filesystem) → **Environment Knowledge** work product + Canary (DevOps, lifts `reference_automemory_scope_behavior`); controlled (`inbox.sh`, layout, `MEMORY.md`, committed pin) → our IaC + health-check; the **seam** → a new lean element **Dependency Contract** (`contracts/memory-substrate.md`). **KB-check:** the contract (invariants *we require*) is genuinely distinct from Environment Knowledge's *dependency chain* (downstream blast radius) — complement, not duplicate. Clauses C1 shared-path resolution · C2 durable across sessions/types · C3 worktree-independent inbox (all hold; C1/C3 live in the session-health hook #117) · **C4 concurrency safety = Decided lean (2026-06-14)**: exposure is narrow (fact files single-owner, inbox messages unique-file; only `MEMORY.md` is a shared hot file) → guarantee held by convention + reconciler (append-own-line + OE `consolidate-memory` + index-integrity check), hard lock rejected as disproportionate; if the integrity check ever trips → revisit (monitored). Mechanization: substrate health-check asserts the clauses, co-located with G2 (DevOps; C1/C3 done #117, C4 index-integrity + C2 assertion pending). EK work product for the uncontrolled face already exists (`environment/claude-code-app.md` v2). Dogfoods the semait "bind central functionality via contract → impacts derivable; avoid Flut" principle on ourselves. Next: DevOps adds the C4 index-integrity assertion to the hook. |
| **Classification gate (mechanization of Controlled Method Rollout)** — the per-PR "breaking for a drifted agent?" label that triggers/skips the barrier needs to be mechanical, else the two-mode rule is documentation not a standard | Built from the 2026-06-14 two-mode decision (ADR-0012 addendum / Practice `controlled-method-rollout.md`) | OE (criteria/scope) + DevOps (mechanism) | **In Use — ENFORCED (2026-06-15)** | **Live on `main` (PR #125, 2026-06-15):** `.github/workflows/classify-gate.yml` runs on every PR, requires a `rolling`/`breaking` label, default-free, uncertainty→breaking; verified (label pass / no-label fail / re-label green without new commit). **Trigger scope = OE's literal path list** `CLAUDE.md`, `docs/superpowers/skills/**`, `agents/**/claude.md`, `.claude/settings.json`, `scripts/**`, `api/cli.py`. **Enforcement: CLOSED (2026-06-15).** User authorized ("go"); DevOps added `Classification (rolling|breaking)` as a **required status check** on `main` (strict=true; the 6 existing checks untouched) → the gate is **ENFORCED**, not advisory: a WoW-PR without a rolling/breaking label is now blocked at merge. First real dogfood: PR #127 (C5 provenance) passed the gate with a `rolling` label. (Side effect, UX-domain: PR #122 predates the gate workflow → blocked by the pending check until rebased, then passes instantly as non-WoW.) **Scope-gap (open, evaluate after a practice window):** method docs under `docs/superpowers/improvement/**` (practices/, kernel, register) are NOT in the trigger, yet that is where breaking *method* changes live — a breaking edit to a Practice/Kernel would today escape the gate. Deliberately kept narrow first (instantiate-through-use / Gall's Law); revisit adding `docs/superpowers/improvement/**` (and `.github/workflows/**`) — change = one-liner in `scripts/classify_gate.py` + test. Next: OE reviews scope expansion after the practice window. |
| **Memory substrate has no provenance — who edited a memory file is unrecoverable** | 2026-06-15: an edit to `project_automemory_migration_status.md` could not be attributed; OE wrongly guessed "the user". Verified root cause: team memory is **not version-controlled** (only `mtime`) **and** all 11 agents + the user write/commit under one identity (`ThorstenDittmar`) — neither filesystem nor git can say *who* | OE | **Decided (lean), 2026-06-15** | **Decision (user, by practice): self-stamp convention** — every agent that writes a memory file stamps `last-edited-by: <slug>` + `last-edited-at` in the frontmatter `metadata` (builds on existing `originSessionId`; gives the *last editor*, the question that arose). Recorded as **Contract clause C5** (`contracts/memory-substrate.md`) and surfaced to all agents in `knowledge-routing.md` (loaded at every anchor). **Rejected (for now):** version-controlling the memory dir — heavier and the shared identity means it still wouldn't attribute without the stamp. **Enforcement honesty:** memory is not in CI and the memory-*writing* behaviour is system-prompt-driven (not an OE-editable surface) → the stamp is **ritual**; the mechanical half is *detection only*. **Update 2026-06-16 (DevOps Essence-audit):** the originally-named `mtime`-based detection is **deferred** — `mtime` advances on non-content ops (converge/checkout/sync) → false positives; it needs a **content/git-based design** (own OE-owned strand, non-blocking). The **self-stamp ritual stands** (C2/C4, by contrast, were specified-but-unenforced (RC1) and are now **live in the hook #133**). Ties to the open shared-blackboard ownership question (the cwd-shared-auto-memory row above). |
| **Barrier-wave execution as a reusable skill** — the lead-and-spawn pattern (cut into packages → spawn fresh sub-agents in isolated worktrees → domain-owner ratifies → fold pre-merge fixes → merge, wave by wave) | F0.1+F0.2 execution (2026-06-16): the pattern recurred ~5× (split waves P1/P4, P2/P3, P5; build waves P-A/B/E, P-C/D, P-F) — near-identical orchestration each time | OE | **Identified** | candidate `barrier-wave-execution` skill; next retro prioritizes |
| **Isolated-worktree-spawn git friction** — sandboxed spawns cannot run `git mv` / `git -C`; a spawn's PR branch stays checked out in its (uncleaned) worktree, forcing the lead onto `oe/pX-fix` side-branches for ratification fixes; stale agent worktrees accumulate under `~/klartext/.claude/worktrees/` | F0.1+F0.2 execution (2026-06-16): hit on every spawn package | OE (method) + DevOps (tooling) | **Identified** | candidates: spawn-prompt git-fallback boilerplate, a worktree-prune step, or lead-direct for tiny fixes; ties to the worktree-cleanup PENDING line; next retro prioritizes |
| **Matrix organisation modelled explicitly + perspective-relative** — the multi-agent system *is* a matrix; today it is modelled flat/implicit (CLAUDE.md role table + domain boundaries), which produces mis-cuts like "verify belongs to UX". Horizontal vs. Vertical are **not absolute** but depend on the viewpoint of the discipline (each agent is a Vertical from its own view, a Horizontal from another's) | user observation 2026-06-17 (F3 restart, reading the `verify` skill): `verify`-as-UX-solo is the **symptom**; the root is the mis-modelled matrix | OE | **Identified** | **deliberately parked — OE design task, only when the user prioritizes; do not start the rebuild.** Full context in memory `project-matrix-organisation`; symptom = PENDING #49 (verify-drift); next retro prioritizes |
| **Sharpen the classification disqualifier (method/product path cut)** — ANY non-generic binding disqualifies an artifact from L3 (generic library), **including German domain prose**, not only code tokens; a token-grep heuristic **undercounts** because prose bindings carry no grep-able token | SA catch during F3 #3b (2026-06-17): the path-classification disqualifier was token-based and missed prose-level product bindings | OE (+ SA for the gate rule) | **Identified** | feeds the path-classification well-formedness gate (ADR-0013); next retro prioritizes |
| **L3 owner fields → role names at State B** — generic library (L3) cards should carry **role names**, not concrete agent slugs, in their owner fields once they reach State B (generic-ready), mirroring the `merge-protocol` card | F3 anchor 2026-06-17 (consistency sweep over L3 cards) | OE | **Identified** | small editorial sweep; pattern already set by `merge-protocol`; next retro prioritizes |
| **Per-file cut-classification of code stems** — if code stems are ever pulled into the method seed, classify them **per file** against the method/product cut (not stem-wholesale), since one stem can mix generic and product-bound files | F3 anchor 2026-06-17 (raised as a conditional, deferred) | OE | **Identified — conditional** | only relevant *if* code stems are ever wanted in the seed; recorded so the decision is not re-derived; next retro prioritizes on that trigger only |
| **qa-review skill: foreign-PR + sub-agent-failure fallback** — the skill instructs only "dispatch a fresh sub-agent against `git diff HEAD`"; two assumptions broke this session: (a) sub-agent dispatch available (was 529-blocked), (b) reviewing the *own* working-tree diff (was a foreign PR branch whose code is not on `agent/qa`); add a fallback — run inline when dispatch hangs + throwaway-worktree for foreign PRs | QA anchor briefing 2026-06-17 (F2 Gate-1 qa-review on #158/#157: DevOps spawn hung 2× in 529, OE directed an inline run) | QA (skill) + OE (register) | **Identified** | SoT is the repo skill copy → propagate via skill-sync; the technique itself is captured in `agents/qa/claude.md`; next retro prioritizes |
| **Restart seed must state the operating mode after a barrier wave** — when an agent boots a fresh session after a coordinated barrier/freeze wave (e.g. F3), the successor seed must explicitly name the mode transition (wave *exited* → steady-state, inbox thawed); otherwise the fresh session carries a stale "barrier still active" model and conflates a lifted, wave-specific freeze with the always-on discipline against unilaterally touching ratified artifacts | SA, fresh post-F3 session 2026-06-17: stated "Barrier-Mode bleibt aktiv: kein eigenmächtiger F3-Eingriff" on PR #164, although F3 had long exited (inbox thawed, steady-state) — the whole session ran on a live inbox. His instinct (flag the ratified ADR-0013 hunk) was right, but mis-attributed to an F3 barrier rather than the standing gated-landing discipline | OE | **Identified** | folds into the **Generation-change practice card** row above (still prose-only): add an explicit "operating mode (wave / steady-state, inbox frozen / thawed)" field to the successor-seed template; next retro prioritizes |
| **Push-before-pull freshness gap → receiver-side warning** — a briefing referencing a just-merged commit can reach a recipient still on a pre-merge snapshot; with no sync signal the recipient evaluates stale state and may even misdiagnose it as a tooling bug | DevOps, 2026-06-17: read the #166 briefing on a pre-#166 worktree, concluded "converge is buggy" before re-converging — retracted after empirical re-test | OE (inbox fabric) + DevOps (converge/landed) | **In Use** | fixed mechanically both halves: `inbox.sh read`/`unread` now warns when behind `origin/main` (fetch-before-count, #168, verified current→silent / behind→warns); sender-side annotation kept as optional Kür. Enforcement-hierarchy honesty: mechanical receiver-side beats sender ritual |
| **Squash hides a landed commit from `--contains`/SHA checks** — with squash-merge a branch commit never becomes an ancestor of main even though its content landed, so `git branch --contains`/SHA "did this land?" checks give false negatives (orphaned-seam illusion) | DevOps, 2026-06-17: `f0fc0b6` (Node-24 bump) looked orphaned by `--contains`, but its content was already on main; second root of the day's false conclusion | DevOps (tool) + OE (register) | **In Use** | mechanically addressed: `klartext landed <ref>` checks by content/patch-id (`git cherry`), squash-aware (#167); proved on `f0fc0b6 → landed by content`. Ties to the squash-default merge policy — verify landing by content/PR-number, never by SHA |
| **WoW-Retrospective on a hybrid cadence + named health grid** — the Retrospective (Advances the Way-of-Working alpha) was event-triggered only, leaving the alpha's *In Use* box "Regularly inspected" unsatisfied; now it fires event-based **and** on a standing milestone/team-refresh backstop, and explicitly walks the WoW-alpha state checklist as its health grid | OE self-assurance deep-dive 2026-06-18 — the WoW-alpha checklist independently pointed at "Regularly inspected" as the next health box | OE | **Action Agreed** | first pillar of the 3-layer OE-assurance model (mechanical method-linter · alpha-checklist · cadence-retro · SA structural peer); card updated (`enactment/practices/retrospective.md`); cadence tuned in use. Next: SA-as-peer (needs SA buy-in) + method-linter invariants → DevOps. See memory `project-oe-self-assurance-open` |
