...
| Compaction monitoring (hooks + launchd digest) | compaction concern (2026-06-09) | DevOps | **Trialed** (2nd trial, after failed 1st) | **retro 1 (2026-06-10): 1st trial FAILED** — launchd fired into a missing script (RC4 on the guard itself). **Re-fix landed (PR #51, `3f6b825`):** script versioned, plist template, setup.sh integration, **3 infrastructure tests** that catch exactly this scenario in every CI run (mechanical). **DevOps involvement-check answer (retro 1, Need 3):** root cause = sequencing error (script lived on `salvage/h01-working-tree`, the promotion branch kept getting deprioritized behind method-foundation increments) + missing existence gate at launchd install time (checked the plist template, not the target script after branch switches) — RC4 + RC2 confirmed. **Evaluate at next retro:** does `compact-monitor.log` show clean hourly runs? |
...
| **Merge protocol for parallel dispatches** — explicit merge owner + PR/rebase order per work package; file-path-disjoint tasks ≠ regulated merge order | H01-422 run (2026-06-10): Hannibal dispatched QA (`api/tests/`) and UX/UI (`frontend/`) in parallel, correctly disjoint, but PR/rebase order on `main` was unregulated (user flagged the risk; Hannibal owns the gap honestly) — RC5 | Hannibal (card support: OE) | **Identified** | retro 2 material; candidate: practice card "Merge Protocol" (merge owner + order as part of every parallel dispatch) |
