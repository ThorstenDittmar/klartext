...
| Compaction monitoring (hooks + launchd digest) | compaction concern (2026-06-09) | DevOps | **Results Evaluated (partial)** — Stage 2 proven, Stage 1 gated | **3rd trial (2026-06-10, PRs #60/#61): Stage 2 (reader half) PROVEN end-to-end** — synthetic `| auto |` entry → digest + notification; idempotent; **the E2E trial caught a REAL bug** (a single `| manual |` entry crashed the script under `set -euo pipefail` and froze state — fixed #61, regression-tested; per-link existence checks would never have seen it → hard confirmation of the "test the chain, not the links" learning). Hook paths hardened to `$CLAUDE_PROJECT_DIR` (#60, Option 0). 8 infra tests across both halves. **Stage 1 (hook fires in real agent sessions) remains gated on the structural decision (cwd/worktree study, Option A/B)** — final trial then. History: 1st trial failed (script missing), 2nd failed (hook layer dead) — see `learnings/2026-06-10-h01-422-retrospective.md` |
...
| **Merge protocol for parallel dispatches** — explicit merge owner + PR/rebase order per work package; file-path-disjoint tasks ≠ regulated merge order | H01-422 run (2026-06-10): Hannibal dispatched QA (`api/tests/`) and UX/UI (`frontend/`) in parallel, correctly disjoint, but PR/rebase order on `main` was unregulated (user flagged the risk; Hannibal owns the gap honestly) — RC5 | Hannibal (card support: OE) | **Trialed** | First live enactment succeeded (2026-06-10): #53→#54→#55, named owner, artifact-verified. **Card written (`practices/merge-protocol.md`) + `method.md` row, NN for parallel dispatches.** Evaluate at next parallel work package: did the card hold as written? |
