...
| **Channel consolidation — inbox is the standing channel, even though the app channel is back** — with the return to the desktop app (2026-06-14) the app messaging fabric (`ccd_session_mgmt`) is available again, so cross-agent messaging now has *two* possible channels (file inbox `scripts/inbox.sh` + app messages). Risk: drifting into two channels by accident | Surfaced during the app-return work (2026-06-14): OE flagged twice that both channels are now live; the user kept using the inbox, but the choice was never recorded as a deliberate decision | OE | **Identified** | **Decision (user, by practice): the file inbox stays the standing cross-agent channel** — it is session-type-agnostic (proven across terminal+app sessions), durable on disk, and the recipient reads it via the user-as-relay. The app channel being available again does not change the channel; inbox is the channel *by choice*, not by lack of an alternative. Open: explicitly frame/justify this (one channel, not two by accident) — e.g. when is an app direct-message ever preferable, and how do we prevent silent dual-channel drift. Captured in the memory [[oe-direct-message-autonomy]]; next retro may promote this from a recorded practice to an explicit rule. |
