...
| **Memory pinning is machine-wide — migrate to project-specific scoping later** — `autoMemoryDirectory` in the user-global `~/.claude/settings.json` redirects ALL sessions on the machine to the klartext team memory | user decision (2026-06-10, Stage 1): accepted deliberately because klartext is currently the only project on this machine — "das kann sich aber ändern" | DevOps (settings mechanics) | **Identified** | revisit before/when a second project appears: project-scoped setting (e.g. via project `.claude/settings.json` once sessions actually load it — depends on the cwd repair / generation change) or per-project memory dirs. Not urgent, but a known time bomb if forgotten |
