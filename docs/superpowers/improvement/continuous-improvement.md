...
| **Classification gate (mechanization of Controlled Method Rollout)** — the per-PR "breaking for a drifted agent?" label that triggers/skips the barrier needs to be mechanical, else the two-mode rule is documentation not a standard | Built from the 2026-06-14 two-mode decision (ADR-0012 addendum / Practice `controlled-method-rollout.md`) | OE (criteria/scope) + DevOps (mechanism) | **In Use (reporting) — enforcement pending one toggle** | **Live on `main` (PR #125, 2026-06-15):** `.github/workflows/classify-gate.yml` runs on every PR, requires a `rolling`/`breaking` label, default-free, uncertainty→breaking; verified (label pass / no-label fail / re-label green without new commit). **Trigger scope = OE's literal path list** `CLAUDE.md`, `docs/superpowers/skills/**`, `agents/**/claude.md`, `.claude/settings.json`, `scripts/**`, `api/cli.py`. **Enforcement gap (pending):** the check `Classification (rolling|breaking)` is **not yet a required status check** in `main` branch protection (repo-admin setting; DevOps deliberately did not set it silently). Until the user flips *Settings → Branches → main → Require status checks → "Classification (rolling|breaking)"*, the rule is **advisory, not enforced** — the enforcement half of our hierarchy is open. **Scope-gap noted (DevOps, evaluate after a practice window):** method docs under `docs/superpowers/improvement/**` (practices/, kernel, register) are NOT in the trigger, yet that is where breaking *method* changes live — a breaking edit to a Practice/Kernel would today escape the gate. Deliberately kept narrow first (instantiate-through-use / Gall's Law); revisit adding `docs/superpowers/improvement/**` (and `.github/workflows/**`) after a few weeks — change = one-liner in `scripts/classify_gate.py` + test. Next: (1) user sets the required-check toggle; (2) OE reviews scope expansion after the practice window. |
