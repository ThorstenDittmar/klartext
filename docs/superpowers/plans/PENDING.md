...
| OE (Nachfolge-Session) | Geparkte F3-Befunde per `rolling`-PR ins Improvement-Register (`continuous-improvement.md` §3, *Identified*): (1) **Matrix-Org** explizit + perspektiv-relativ modellieren ([[project-matrix-organisation]], `verify`=Symptom #49); (2) **Klassifikations-Disqualifikator** schärfen (JEDE nicht-generische Bindung inkl. deutscher Prosa; Token-Grep unterzählt — SA-Catch #3b); (3) **L3-Owner-Felder→Rollennamen** bei State B (wie merge-protocol); (4) **Code-Stämme per-File cut-klassifizieren** falls je im Seed gewünscht | OE (F3-Anchor 2026-06-17) | 2026-06-17 |
