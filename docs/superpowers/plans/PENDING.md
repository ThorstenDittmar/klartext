...
| ~~OE~~ | ~~Fake-Ownership in `api/tests/fakes/` klären (`fake_narrative_repository.py`: NE-Interface, kein Owner)~~ — **erledigt 2026-06-17 (dieser PR)**: Regel in CLAUDE.md §Test-helper ownership (Helfer → Interface-Eigner-Zelle, QA = Contract); fakes + mothers per-Domain in die Agenten-Write-Access eingetragen; wirkgefuege-Fake→CM mit Split-Notiz; mechanische Enforcement (CODEOWNERS-artig) = separates DevOps-Item | Audit Expert | 2026-06-10 |
...
| DevOps | ADR-0014 Agent-Provenance-Hook **GEBAUT ✅ (#161, F3-#2, gemergt 2026-06-17)**: commit-msg-Hook (`scripts/agent_trailer.py`, inject+validate) + `agent-provenance.yml` CI-Gate, spawn-aware, `Agent: human`-Bypass. **Rest-Folge (low-prio):** commit-msg-Hook einmalig auf der shared-`core.hooksPath`-dev-Maschine installieren (`pre-commit install` verweigert bei gesetztem hooksPath) — CI enforced serverseitig ohnehin. ADR-0014-Status Proposed→Accepted = SA-Flip. | System Architect | 2026-06-16 |
...
| UX (+ QA-Kriterien) | `verify`-Skill-Drift nachziehen (`docs/method/enactment/skills/verify.md`) + L2-Karte synchronisieren: (a) 5-Screen-URLs veraltet ggü. App.tsx (`/reading` nicht gemountet; `/causal-model`+`/narrative` brauchen IDs; `NarrativeEditor`→`NarrativeDetail`); (b) Browser-MCP-Reihenfolge — Claude_Preview primär, Claude_in_Chrome Fallback (localhost-Sperre) — **tieferer Befund (2026-06-17): das ist nur das Symptom; `verify` ist eine QA×UX-Matrixzelle, kein UX-Solo → [[project-matrix-organisation]]** | OE (F0.2-P-D, UX-geflaggt #150) | 2026-06-16 |
| DevOps | Stale Agent-Spawn-Worktrees aufräumen: nach den F0.1/F0.2-Spawns liegen ~7 Worktrees unter `~/klartext/.claude/worktrees/agent-*` (Branches bereits gemerged+gelöscht auf origin) — `git worktree prune` / `remove`. **2026-06-17: real 11; DevOps hat Entfernung angeboten, OE-„ja"** | OE (F0.2-Anchor 2026-06-16) | 2026-06-16 |
| SA | ADR-0014 Status `Proposed→Accepted` flippen (Hook+CI seit #161 live auf main) — dabei C5-Wording-Präzision: der Trailer ist das **git-commit-Geschwister** von C5, NICHT „generalises C5" (Memory-File-Provenance bleibt self-stamp, liegt außerhalb git) | OE (F3-Anchor 2026-06-17) | 2026-06-17 |
| ~~OE (Nachfolge-Session)~~ | ~~Geparkte F3-Befunde per `rolling`-PR ins Improvement-Register (`continuous-improvement.md` §3, *Identified*): (1) **Matrix-Org**; (2) **Klassifikations-Disqualifikator**; (3) **L3-Owner-Felder→Rollennamen**; (4) **Code-Stämme per-File cut-klassifizieren**~~ — **erledigt 2026-06-17 (dieser PR)**: 5 Zeilen in §3 *Identified* (die 4 + **qa-review-Skill-Gap** aus QA-Anchor-Briefing) + QA-Technik „fremde PRs & Sub-Agent-Ausfall" in `agents/qa/claude.md` | OE (F3-Anchor 2026-06-17) | 2026-06-17 |
