...
| G4 | **Allowlist patterns match the real invocation form** — `Bash(semgrep*)` covers the bare call but **not** the real `…/api/.venv/bin/semgrep` absolute path the pre-commit hooks use, so the genuine invocations would prompt. Refine allowlist patterns to the actual (venv-absolute) form. | DevOps + SA | OPEN |
